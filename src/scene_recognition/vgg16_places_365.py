...
if __name__ == '__main__':
    # open the first 5 images in the places_small folder
    images = []
    for file in os.listdir("../../data/places_small/"):
        image = Image.open(os.path.join("../../data/places_small/", file))
    ...
    top_preds = np.argsort(preds, axis=1)[:, ::-1][:,0:predictions_to_return]
    ...
    labels = { idx : "\n".join([classes[top_preds[idx, i]] for i in range(3)]) for idx in range(top_preds.shape[0]) }
